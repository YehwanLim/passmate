...
- **샘플 리포트**: `/company-report?sample=1`(12함수 한도로 라우트를 늘리지 않고 쿼리로 구분)에 실제 생성 결과를 굳힌 상수(`client/src/constants/companyReportSample.ts`)를 동일 컴포넌트로 렌더. 상단 리본 "샘플 리포트 · {회사} · {기준일} 기준".
