...
function CreditSkeleton() {
  return (
    <div className="border-y border-white/5" aria-label="이용권 정보를 불러오는 중">
      {[1, 2, 3].map((row) => (
        <div
          key={row}
...
export default function Entitlements() {
  ...
  const { isLoading: authLoading, isAuthenticated } = useRequireAuth({ redirectPath: "/entitlements" });
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] pb-28 text-white">
        ...
        <div className="container flex h-16 items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              aria-label="PassMate 홈"
            >
              <Logo className="h-6 w-6" textClassName="text-lg md:text-xl text-white" />
            </button>
          </div>
          <AuthButton />
        </div>
      ...
      <main className="container max-w-3xl pt-10 pb-8">
        ...
        >
          <h1 className="text-2xl font-bold text-zinc-100 tracking-tight mb-1">이용권</h1>
            ...
            <div className="space-y-4">
              ...
              <div className="border-y border-white/5">
                <div className="flex items-center justify-between gap-5 py-4">
                  <div>
                    <p className="text-sm font-medium text-zinc-300">남은 분석</p>
