...
async function getSummaryForEntitlement(tx, userId, entitlement) {
  ...
  // companyAnalysisEnabled 스위치는 일반 사용자 생성·판매를 막을 뿐 이미 지급된
  // 크레딧 잔액에는 영향을 주지 않는다(위 BONUS 와 같은 이유 — 관리자 QA 는 스위치와 무관하다).
  const companyAnalysisEnabled = settings?.companyAnalysisEnabled === true;
  const companyUsed = await getUsage(tx, userId, "PREMIUM", "COMPANY");
  const companyRemaining = Math.max(
    Number(entitlement?.companyCreditsGranted ?? 0) - companyUsed,
    0,
  );
...
/**
 * 화면 표시용 조회 전용 요약. 크레딧을 차감하지 않는 경로에서 쓴다.
 *
 * getEntitlementSummary 와 달리 행 잠금(FOR UPDATE)·upsert 없이 모든 조회를
 * 병렬로 실행한다 — 예약/차감처럼 정합성이 필요한 경로는 반드시 잠금 버전을 쓸 것.
 * entitlement 행이 아직 없는 계정(첫 분석 전)은 보너스·프리미엄 0으로 본다.
 */
export async function getEntitlementSummaryReadOnly(db, userId) {
  ...
  const premiumRemaining = premiumEnabled
    ...
    : 0;
  // companyAnalysisEnabled 스위치는 일반 사용자 생성·판매를 막을 뿐 이미 지급된
  // 크레딧 잔액에는 영향을 주지 않는다(관리자 QA 는 스위치와 무관하다).
  const companyAnalysisEnabled = settings?.companyAnalysisEnabled === true;
  const companyRemaining = Math.max(
    Number(entitlement?.companyCreditsGranted ?? 0) - companyUsed,
    0,
