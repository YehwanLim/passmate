...
const MAX_POSTING_FIT_ITEMS = 6;

/** postingFit 의 목록 상한. 지시만으로는 키워드 10개가 나와(09-15 실측) 서버가 자른다. */
export function tidyPostingFit(report) {
  if (!isRecord(report) || !isRecord(report.postingFit)) return report;
  const fit = report.postingFit;
  const capped = { ...fit };
  if (Array.isArray(fit.requirementMatches)) capped.requirementMatches = fit.requirementMatches.slice(0, MAX_POSTING_FIT_ITEMS);
  if (Array.isArray(fit.missingKeywords)) {
    capped.missingKeywords = [...new Set(fit.missingKeywords.filter((k) => typeof k === "string" && k.trim()))].slice(0, MAX_POSTING_FIT_ITEMS);
  }
  return { ...report, postingFit: capped };
}
...
export async function analyzeCoverLetter(request, db = prisma) {
  ...
  for (let attempt = 0; attempt < candidates.length; attempt += 1) {
    ...
    try {
      ...
      return {
        ...tidyPostingFit(attachRequestAnswers(result.parsed, request.questions)),
        analysisMeta: {
