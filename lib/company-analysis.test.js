...
describe("grounding metadata", () => {
  ...
  it("numbers unique web sources from 1 and drops non-web chunks", () => {
    expect(extractGroundingSources(data)).toEqual([
      { id: 1, title: "hyundai.com", url: "https://redirect/1", publisher: "redirect", excerpt: null },
      { id: 2, title: "dart.fss.or.kr", url: "https://redirect/2", publisher: "redirect", excerpt: null },
    ]);
  });
    ...
    candidates: [{
      ...
      groundingMetadata: grounding
        ? {
          ...
          groundingSupports: custom?.supports,
        }
        : undefined,
    }],
    usageMetadata: usage,
...
describe("analyzeCompany", () => {
  it("returns the grounded report with sources, meta, and usage after one call", async () => {
    ...
    expect(result.sources).toEqual([{ id: 1, title: "hyundai.com", url: "https://redirect/1", publisher: "redirect", excerpt: null }]);
...
describe("analyzeCompany two-stage pipeline", () => {
  ...
  it("runs two research calls in parallel, then composes JSON without tools from both memos, merging the sources", async () => {
    ...
    expect(composeText).toContain("의혹");
    expect(composeText).toContain("기준일과 가까운 날짜");
...
describe("grounding source excerpts", () => {
  const redirect = (n) => `https://vertexaisearch.cloud.google.com/grounding-api-redirect/${n}`;
  const grounded = (chunks, supports) => ({
    candidates: [{ groundingMetadata: { groundingChunks: chunks, groundingSupports: supports } }],
  });

  it("attaches the supported memo line to each source and names the publisher by the site, not the redirect host", () => {
    const sources = extractGroundingSources(grounded(
      [{ web: { uri: redirect("a"), title: "imbc.com" } }, { web: { uri: redirect("b"), title: "samsung.com" } }],
      [
        { segment: { endIndex: 115, text: "- (2026-07) 삼성전자 2분기 매출은 171조 4,995억 원을 기록했다." }, groundingChunkIndices: [0] },
        { segment: { startIndex: 116, endIndex: 291, text: "— MBC 뉴스, Samsung Newsroom\n- (2026-07) 이는 역대 최대 분기 실적이다. — Samsung Newsroom" }, groundingChunkIndices: [1] },
      ],
    ));
    expect(sources).toEqual([
      { id: 1, title: "imbc.com", url: redirect("a"), publisher: "imbc.com", excerpt: "(2026-07) 삼성전자 2분기 매출은 171조 4,995억 원을 기록했다." },
      { id: 2, title: "samsung.com", url: redirect("b"), publisher: "samsung.com", excerpt: "(2026-07) 이는 역대 최대 분기 실적이다." },
    ]);
  });

  it("keeps the first excerpt for a source cited several times, trims long ones, and leaves excerpt null without supports", () => {
    const long = `- ${"가".repeat(200)}`;
    const sources = extractGroundingSources(grounded(
      [{ web: { uri: redirect("a"), title: "a.com" } }, { web: { uri: redirect("b"), title: "b.com" } }],
      [
        { segment: { text: "- 첫 번째 사실" }, groundingChunkIndices: [0] },
        { segment: { text: "- 두 번째 사실" }, groundingChunkIndices: [0] },
        { segment: { text: long }, groundingChunkIndices: [1] },
      ],
    ));
    expect(sources[0].excerpt).toBe("첫 번째 사실");
    expect(sources[1].excerpt.length).toBeLessThanOrEqual(140);
    expect(sources[1].excerpt.endsWith("…")).toBe(true);
    expect(extractGroundingSources(grounded([{ web: { uri: redirect("c"), title: "c.com" } }], undefined))[0].excerpt).toBeNull();
  });

  it("spreads shared sentences across sources and strips the date placeholder", () => {
    const sources = extractGroundingSources(grounded(
      [{ web: { uri: redirect("a"), title: "a.com" } }, { web: { uri: redirect("b"), title: "b.com" } }, { web: { uri: redirect("c"), title: "c.com" } }],
      [
        { segment: { text: "- (2025-11) 사업지원실을 상설화했다." }, groundingChunkIndices: [0, 1, 2] },
        { segment: { text: "- (2025-11) M&A팀을 신설했다." }, groundingChunkIndices: [1, 2] },
        { segment: { text: "- (YYYY) 기획팀은 중장기 전략을 수립한다." }, groundingChunkIndices: [2] },
      ],
    ));
    expect(sources.map((source) => source.excerpt)).toEqual([
      "(2025-11) 사업지원실을 상설화했다.",
      "(2025-11) M&A팀을 신설했다.",
      "기획팀은 중장기 전략을 수립한다.",
    ]);
  });

  it("strips asterisk bullets from memo lines", () => {
    const sources = extractGroundingSources(grounded(
      [{ web: { uri: redirect("a"), title: "a.com" } }],
      [{ segment: { text: "* (2026-08) 삼성전자의 사업은 DS 와 DX 로 나뉜다." }, groundingChunkIndices: [0] }],
    ));
    expect(sources[0].excerpt).toBe("(2026-08) 삼성전자의 사업은 DS 와 DX 로 나뉜다.");
  });

  it("alternates between the two research calls so neither group is starved by the cap", async () => {
    const chunks = (prefix, count) => Array.from({ length: count }, (_, index) => ({ web: { uri: redirect(`${prefix}${index}`), title: `${prefix}${index}.com` } }));
    const { fetcher } = recordingFetcher([
      geminiResponse({ text: "- 사업", grounding: { chunks: chunks("b", 18) } }),
      geminiResponse({ text: "- 직무", grounding: { chunks: chunks("r", 18) } }),
      geminiResponse({ text: JSON.stringify(REPORT), grounding: false }),
    ]);
    const result = await analyzeCompany(request, db, { fetcher, apiKey: "test-key" });
    expect(result.sources).toHaveLength(20);
    expect(result.sources.map((source) => source.title).slice(0, 4)).toEqual(["b0.com", "r0.com", "b1.com", "r1.com"]);
    expect(result.sources.filter((source) => source.title.startsWith("r")).length).toBe(10);
    expect(result.sources.map((source) => source.id)).toEqual(Array.from({ length: 20 }, (_, index) => index + 1));
  });

  it("merges sources from both research calls by url, keeping an excerpt from whichever call had one", async () => {
    const { fetcher } = recordingFetcher([
      geminiResponse({ text: "- 사업 사실", grounding: { chunks: [{ web: { uri: redirect("x"), title: "x.com" } }], supports: [{ segment: { text: "- 사업 사실" }, groundingChunkIndices: [0] }] } }),
      geminiResponse({ text: "- 직무 사실", grounding: { chunks: [{ web: { uri: redirect("y"), title: "y.com" } }, { web: { uri: redirect("x"), title: "x.com" } }], supports: [{ segment: { text: "- 직무 사실" }, groundingChunkIndices: [0, 1] }] } }),
      geminiResponse({ text: JSON.stringify(REPORT), grounding: false }),
    ]);
    const result = await analyzeCompany(request, db, { fetcher, apiKey: "test-key" });
    expect(result.sources).toEqual([
      { id: 1, title: "x.com", url: redirect("x"), publisher: "x.com", excerpt: "사업 사실" },
      { id: 2, title: "y.com", url: redirect("y"), publisher: "y.com", excerpt: "직무 사실" },
    ]);
  });
});
