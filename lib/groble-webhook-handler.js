// Groble 결제 웹훅 처리기. 12함수 한도 때문에 전용 api 파일 대신
// api/entitlements.js 가 rewrite(/api/webhooks/groble)로 이 핸들러에 위임한다.
import { createHash } from "node:crypto";

import { grantGroblePurchase } from "./analysis-entitlements.js";
import { PURCHASE_PRODUCTS } from "./entitlement-products.js";
import {
  GrobleWebhookError,
...
function createSafeDiagnostic({ body, code, event, headers }) {
  ...
  return {
    ...
    contentId: createIdentifierDiagnostic(dataObject?.content?.id),
...
function resolveProductForContentId(contentId, premiumContentId, singleContentId) {
  if (contentId === premiumContentId) {
    return "TRIPLE";
  }
...
export function parseGroblePaidEvent(body, premiumContentId, singleContentId) {
  if (!isRecord(body)) {
  ...
  const product = resolveProductForContentId(
    object.content.id,
    premiumContentId,
    singleContentId
  );
...
export function createGrobleWebhookHandler({
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      event = parseGroblePaidEvent(body, premiumContentId, singleContentId);
      ...
      const grant = await prismaClient.$transaction(async (tx) => {
        const purchaseIntent = await tx.purchaseIntent.findUnique({
          ...
          select: { id: true, product: true, status: true, userId: true },
        });
        ...
        // 지급 크레딧은 실제 결제된 상품(웹훅 contentId) 기준이다. 의도와 결제가
        // 어긋나도 지불액과 지급이 항상 일치하므로 지급하되, 추적을 위해 남긴다.
        if (purchaseIntent.product !== event.product) {
          logger("[api/webhooks/groble] purchase intent product mismatch", {
            correlationId: createCorrelationId(event),
            intentProduct: purchaseIntent.product,
            paidProduct: event.product,
          });
        }

        return grantGroblePurchase(tx, {
          credits: PURCHASE_PRODUCTS[event.product].credits,
