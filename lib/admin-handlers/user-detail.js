...
export default createAdminHandler({ route: "api/admin/users/[id]", methods: ["GET"] }, async ({ req, requestId, res }) => {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: {
      ...
      paymentEntitlements: {
        ...
        select: { id: true, creditsGranted: true, providerPaymentId: true, rawEvent: true, createdAt: true },
      },
      // 결제창까지 갔다가 완료되지 않은 시도. PAID 는 위 결제 내역에 이미 나온다.
      purchaseIntents: {
        where: { status: { not: "PAID" } },
        orderBy: { createdAt: "desc" }, take: 20,
        select: { id: true, product: true, status: true, createdAt: true },
      },
    },
  });
  if (!user) return sendRequestError(res, 404, requestId);
  // 상품 구분이 없던 옛 결제 기록은 contentId 로 상품을 되짚는다.
  const contentIds = contentIdsBySettings(await readPurchaseProductSettings(prisma));
  ...
  return res.status(200).json({
    ...
    payments: user.paymentEntitlements.map((payment) => ({
      id: payment.id,
      product: resolveProductForPaymentRecord(payment.rawEvent, contentIds),
      credits_granted: payment.creditsGranted,
      provider_payment_id: payment.providerPaymentId,
      created_at: payment.createdAt,
    })),
    pending_purchases: user.purchaseIntents.map((intent) => ({
      id: intent.id, product: intent.product, status: intent.status, created_at: intent.createdAt,
    })),
