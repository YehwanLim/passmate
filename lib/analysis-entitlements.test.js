...
describe("analysis entitlements", () => {
  ...
  it("keeps admin-granted company credits usable while the company analysis switch is off", async () => {
    ...
    expect(summary.companyRemaining).toBe(1);
    await expect(reserveAnalysis(db, USER_ID, "COMPANY")).resolves.toMatchObject({ kind: "COMPANY" });
  });
});
