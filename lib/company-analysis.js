...
다음은 검색으로 조사한 메모입니다. 이 메모의 사실만 사용해 마스터 프롬프트의 JSON 스키마로 리포트를 작성하세요. 메모에 없는 사실을 추가하지 마세요. "[계열사 이름]"이 붙은 줄은 이 회사의 사업으로 쓰지 말고, 이 회사에 직접 영향을 줄 때만 계열사 이름을 밝혀 currentIssues 에 씁니다. 계열사의 공시·차입·배당은 recentDisclosures 에 넣지 않습니다. 메모에 없는 주제(예: 인재상 "확인하지 못함")는 해당 배열을 비웁니다. 메모의 출처 도메인은 sourceIds 번호 근거로만 쓰고 본문에 옮기지 않습니다.
