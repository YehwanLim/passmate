...
다음은 검색으로 조사한 메모입니다. 이 메모의 사실만 사용해 마스터 프롬프트의 JSON 스키마로 리포트를 작성하세요. 메모에 없는 사실을 추가하지 마세요. "[계열사 이름]"이 붙은 줄은 이 회사의 사업으로 쓰지 말고, 이 회사에 직접 영향을 줄 때만 계열사 이름을 밝혀 currentIssues 에 씁니다. 계열사의 공시·차입·배당은 recentDisclosures 에 넣지 않습니다. 메모에 없는 주제(예: 인재상 "확인하지 못함")는 해당 배열을 비웁니다. 메모의 출처 도메인은 sourceIds 번호 근거로만 쓰고 본문에 옮기지 않습니다. interviewPrep.primarySources 에는 메모에 실제 주소(https://…)가 적힌 자료만 넣고, 주소가 없으면 그 항목을 뺍니다(빈 배열 허용).
...
/** 두 조사 응답의 groundingChunks 를 이어 붙여 한 출처 목록으로 만든다(중복·상한은 extractGroundingSources 가 처리). */
function mergeGroundingSources(datas) {
  const chunks = datas.flatMap((data) => data?.candidates?.[0]?.groundingMetadata?.groundingChunks ?? []);
  return extractGroundingSources({ candidates: [{ groundingMetadata: { groundingChunks: chunks } }] });
}
...
async function analyzeCompanyTwoStage(request, { fetcher, now, apiKey, reportDate, startedAt, deadlineAt }) {
  ...
  return {
    ...
    analysisMeta: {
      ...
      pipeline: "two-stage",
      stages: [
        { name: "research-business", modelName: COMPANY_RESEARCH_MODEL, responseTimeMs: business.responseTimeMs },
        { name: "research-role", modelName: COMPANY_RESEARCH_MODEL, responseTimeMs: role.responseTimeMs },
